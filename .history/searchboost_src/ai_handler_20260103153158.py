...
class AIHandler:
    def __init__(self,logger=None,reason):
        self.logger = logger
    ...
    async def query_LLM(self, ChatDetails):
        ...
        try:
            ...
            ChatDetails.prompt = "User Question : " + ChatDetails.prompt

            if reason == "optimization":
                ChatDetails.prompt = self.query_optimization_prompt + ChatDetails.prompt
            elif reason == "research":
                ChatDetails.prompt = self.query_system_instruction + ChatDetails.prompt
            else:
                self.logger.error(f"Unknown reason for LLM query: {reason}")
            ...
            if (ChatDetails.model.lower() == "local" or "llama" in ChatDetails.model.lower()):
                self.logger.info(f"Using local AI for query {reason}.")
                return ChatDetails.prompt
            else:

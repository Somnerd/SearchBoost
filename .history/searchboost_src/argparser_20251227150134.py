...
async def final_arguments():
    ...
    logging.basicConfig(level=logging.DEBUG,encoding='utf-8', format='%(asctime)s - %(levelname)s - %(message)s',filename='searchboost.log', filemode='a')

    for arg_name , arg_value in vars(args).items():
        logging.info(f"{arg_name}: {arg_value}")

    if args.query is None:
        logging.warning("No query provided via command line. Prompting for input.")

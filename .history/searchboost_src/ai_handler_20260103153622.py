...
class AIHandler:
    ...
    async def query_LLM(self, ChatDetails):
        ...
        try:
            ...
            ChatDetails.prompt =  + ChatDetails.prompt

            if self.reason == "optimization":
                ChatDetails.prompt = self.query_optimization_prompt + "User Question : " + ChatDetails.prompt
            elif self.reason == "research":
                ChatDetails.prompt = self.query_system_instruction + "User Question : "+ ChatDetails.prompt
            else:

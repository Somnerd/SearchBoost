...
async def main():
    try:
        ...
        optimized_query = await ai_handler().optimizer_query(chat_details)
        logger.info(f"Optimized Query: {optimized_query}")
        logger.info("Fetching search results...")
        logger.info("Summarizing search results...")
        web_scraper_instance = web_scraper(optimized_query)
        logger.info(f"Summary:{await web_scraper_instance.searxng_search()}")
